...
class Upload {
    ...
    foreach (array_keys($_FILES) as $varname) {
      ...
      if (is_array($_FILES[$varname]['name'])) {
        ...
        for ($i = 0; $i < $varnameFileCount; $i++) {
          ...
          $this->files[$varname][$fileCount]['ext_filter'] = File::getExtension($_FILES[$varname]['name'][$i]);
        ...
        if ($_FILES[$varname]['name'] != null) {
          ...
          $this->files[$varname][0]['ext_filter'] = File::getExtension($_FILES[$varname]['name']);
  ...
  public function rename($newname, $varname = null) {
    if ($varname !== null) {
      ...
      for ($i = 0; $i < $fileCount; $i++) {
        $ext = File::getExtension($this->files[$varname][$i]['name']);
      ...
      if (is_array($this->files[$varname])) {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
          ...
          if ($this->files[$varname][$i]['error'] == 0) {
            ...
            if (File::validateExtension($this->files[$varname][$i]['name'], $this->files[$varname][$i]['ext_filter'])) {
